...
int main() {
  ...
  ghpi::Action op_auto_fan("OperateAutonomous", ghpi::ActionFn::AFN_OP_AUTO, NULL, "Fan", true);
  { // Code block for scoped_lock. Mutex will automatically unlock after block.
    ...
    // Put the action in the shared memory object
		data->Put(op_auto_fan);
  }
  
  std::cout << "Added Action " << op_auto_fan.get_name() << " to queue" << std::endl;
