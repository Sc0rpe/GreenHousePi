...
int main() {
  ...
  ghpi::Action op_auto_lamp("OperateAutonomous", ghpi::ActionFn::AFN_OP_AUTO, NULL, "Lamp", true);
  { // Code block for scoped_lock. Mutex will automatically unlock after block.
    ...
    // Put the action in the shared memory object
		data->Put(op_auto_lamp);
  }
  
  std::cout << "Added Action " << op_auto_lamp.get_name() << " to queue" << std::endl;
